...
description: "How to manage memory, state, retrieval, and prompts in a live phone call — every layer, with code and diagrams"
---

# Context Engineering for Voice Agents

Context engineering is the practice of intentionally designing and managing the information
you give an LLM — what it is, how it is structured, and when it arrives — so the model's
outputs stay accurate, relevant, and safe.
...
This article breaks down every layer — short-term memory, long-term memory, state, RAG,
prompts, drift, and PII — with diagrams and code from a real four-agent phone system.
...
### Two interventions: truncation and compression

```mermaid
flowchart TD
    A[New message arrives] --> B{Context usage?}
    B -->|below 70%| C[Continue normally]
    B -->|≥ 70% of safe limit| D[Try compression]
    D -->|LLM available| E[Summarise old turns<br/>keep last 4 verbatim]
    D -->|LLM unavailable| F[Truncation fallback<br/>keep last 6 messages]
    G[Agent transfer] --> F
    E --> H[Prepend summary as system message]
    F --> H
```
...
Truncation fires on every agent transfer. The incoming agent gets the last 6 messages —
enough to continue the conversation without dragging the full history forward.
...
Different models in the same call have different limits.

| Agent role | Model | Context limit |
|---|---|---|
| Greeter | Groq Llama 3.1 | 8,000 tokens |
| Reservation | Claude Haiku 3.5 | 200,000 tokens |
| Takeaway | Claude Haiku 3.5 | 200,000 tokens |
| Checkout | Claude Sonnet 3.5 | 200,000 tokens |

The greeter's window is the tightest by a factor of 25. It should receive the leanest
context possible — no menu, no order history, no payment state. Its job is routing, and
routing takes three or four turns.
...
### What belongs in long-term memory for a voice agent

- Past interactions (caller's usual order, booking history)
- Known preferences (allergens, seating preference, language)
- Identity (name, phone number — so the greeter can address them immediately)
...
sequenceDiagram
    ...
    SIP->>Session: Call arrives
    Session->>DB: Fetch caller profile (by phone number)
    DB-->>Session: Profile returned (<300ms)
    Session->>Greeter: Start with profile injected
    Greeter->>Caller: "Welcome back, Amara!"
```
...
the takeaway agent hands off to checkout, checkout does not read the conversation — it reads
the state.
...
flowchart LR
    ...
    State -->|name, phone| Greeter
    State -->|name, phone\nreservation| Reservation
    State -->|name\norder| Takeaway
    State -->|name, phone\norder, payment_status| Checkout
```

When an agent enters, it receives only the state fields it needs. This is not just a
privacy measure — it prevents the model from reasoning about things outside its scope.
...
The checkout agent reads this and knows exactly where it is picking up. It does not need
to ask what the order is. It does not need to re-read the conversation.
...
```mermaid
flowchart TD
    A[Session start] -->|once| B[Static RAG\nMenu, policy, FAQs\ncached at startup]
    C[Caller speaks] --> D{Intent needs\nexternal data?}
    D -->|No| E[Model responds directly]
    D -->|Yes| F[Dynamic RAG\nTool call fires\nfetches data mid-turn]
    F --> G[Result injected\nas function output]
    G --> E
```
...
Inject only into agents that need it. The takeaway agent needs the menu. The greeter and
reservation agent do not. Every token injected into the greeter's 8K window is a token it
cannot use for conversation.
...
Well-designed voice agent prompts average under 100 tokens each. That leaves the vast
majority of even an 8K window for the conversation.
...
You are the reservations specialist at this restaurant. Your job is
...
**Short calls are the best defence.** Each agent in a multi-agent system should own a
narrow slice of the call. A greeter routes in two or three turns. A reservation agent
collects three fields. No single agent runs long enough to drift significantly.

```mermaid
flowchart LR
    Greeter -->|2-3 turns| Reservation
    Greeter -->|2-3 turns| Takeaway
    Takeaway -->|order complete| Checkout
    Reservation -->|done| Greeter
    Checkout -->|done| End([Call ends])
```

**Re-inject identity on every entry.** For agents that run longer, a brief identity note
in the system message on `on_enter` anchors the model:

```
You are the takeaway order taker at Wingu Restaurant.
```
...
over the course of a call is usually drifting. Rising average length before a word-count
threshold is worth monitoring in your observability tool.
...
- Only the checkout agent's `AGENT_CONTEXT_FIELDS` includes payment — and even then
  it receives only `payment_status: pending/complete`, not the card details themselves
...
- Any PII in a compressed summary (compression prompts should explicitly exclude PII
  fields — they live in typed state objects, not in chat messages)
...
```mermaid
flowchart TD
    STM[Short-term memory\nTruncate on transfer\nCompress at 70%]
    LTM[Long-term memory\nPre-fetch before first turn\n< 300ms budget]
    State[State management\nWrite via tools\nRead via scoped injection]
    RAG[RAG\nStatic at startup\nDynamic via tool calls]
    Prompt[Prompt engineering\nShort · stable · runtime separate]
    Drift[Drift prevention\nNarrow agent scope\nRe-inject identity]
    PII[PII handling\nSecretStr · mask before log\nScope by agent]

    STM & LTM & State & RAG & Prompt & Drift & PII --> Core

    Core["Give the model exactly what it needs\nfor this turn, nothing more.\nTrust the state object to hold everything else."]
```
